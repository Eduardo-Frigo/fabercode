# Faber Code v1.0.4 — install updates inside the app

**Official website:** [www.fabercode.site](https://www.fabercode.site/)

This release adds an in-app download and installation flow for macOS, Windows, and Linux on x64 and ARM64.
...
- On Windows, the app starts the matching NSIS installer after verification. On Linux, an AppImage started from a writable location is replaced and relaunched.
- The confirmation and progress text now describe the actual in-app installation flow.

**One-time transition:** v1.0.3 and older do not contain this updater. Install v1.0.4 once from this release page to enable in-app installation for later updates. The previously installed v1.0.2 cannot repair its updater through release metadata alone.
...
| System | x64 | ARM64 |
| --- | --- | --- |
| macOS | `Faber.Code-1.0.4-x64.dmg` | `Faber.Code-1.0.4-arm64.dmg` |
| Windows | `Faber.Code-Setup-1.0.4-x64.exe` | `Faber.Code-Setup-1.0.4-arm64.exe` |
| Linux | `Faber-Code-1.0.4-x86_64.AppImage` | `Faber-Code-1.0.4-arm64.AppImage` |

The [landing page](https://www.fabercode.site/) reads the newest published GitHub release and will display v1.0.4 after all six installers and the checksum file are published.
