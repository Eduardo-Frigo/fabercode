...
6. Confirm `LICENSE` uses Apache-2.0 and `NOTICE` includes Faber Code copyright, attribution and trademark/brand guidance.
7. Confirm third-party names, licenses and links are attributed without implying sponsorship or endorsement.
8. Run `npm run audit:public`.
9. Run `npm run test:architecture` when dependencies are installed.
10. Review `git status --short --ignored` before the first commit.
11. Create commits only after the audit and tests pass.
12. Push from the local repository instead of editing tracked files directly in the GitHub web UI.
