(function () {
  function createWelcomeProjectModalController(options = {}) {
    ...
    function findSelectedProject() {
      ...
      return getRows().find((project) => project && project.id === selectedProjectId) || null;
    }

    async function startFromSelectedProject() {
      ...
      close();
      await onStartProject(selectedProject.id);
    }
    ...
    function bindEvents() {
      if (elements.startConversation) {
        elements.startConversation.addEventListener('click', startFromSelectedProject);
      }
