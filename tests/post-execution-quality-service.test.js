...
async function run() {
  ...
  try {
    ...
    const idempotentEffect = service.evaluateExecutionEffectGate(
      {
    ...
    assert.strictEqual(idempotentEffect.shouldBlock, true);
    assert.ok(idempotentEffect.reason.includes('execution_zero_diff_on_edit'));
    ...
    const artifactWarningService = createPostExecutionQualityService({
      ...
      evaluateOperationBatchArtifactQuality: ({ operations = [] } = {}) => {
        artifactOperationsSeen = operations;
        return {
          enabled: true,
          score: 35,
          minScore: 70,
    ...
    assert.deepStrictEqual(artifactOperationsSeen.map((operation) => operation.path), ['index.html', 'style.css']);
