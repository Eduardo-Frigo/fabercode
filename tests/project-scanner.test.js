...
function run() {
  ...
  try {
    ...
    assert.strictEqual(info.totalFiles, 4);
    assert.strictEqual(info.scannedFiles, 4);
    ...
    const cappedScanner = createProjectScanner({
      excludedDirs: new Set(['node_modules', '.git']),
      fs,
      maxFilesScan: 2,
      path,
    ...
    const cappedInfo = cappedScanner.scanProject(tempRoot);
    assert.strictEqual(cappedInfo.totalFiles, 4);
    assert.strictEqual(cappedInfo.scannedFiles, 2);
    assert.strictEqual(cappedInfo.scannedLimit, 2);
    assert.strictEqual(cappedInfo.truncated, true);
    assert.strictEqual(cappedInfo.files.length, 2);
    assert.strictEqual(cappedInfo.counters.tsx, 1);
    assert.strictEqual(cappedInfo.counters.css, 1);
    assert.strictEqual(cappedInfo.counters.md, 1);
    ...
    try {
      ...
      assert.strictEqual(metadataOnly.scannedFiles, 0);
      assert.strictEqual(metadataOnly.truncated, false);
