...
async function run() {
  ...
  assert.strictEqual(autonomous.meta.workingBriefDomain, 'general-institutional-site');
  assert.strictEqual(autonomous.meta.workingBriefStack, 'next-tailwind');
  assert.strictEqual(autonomous.meta.buildMode, 'adaptive_blueprint');
  assert.strictEqual(autonomous.meta.routeModeContract.productRouteMode, 'adaptive_blueprint');
  assert.strictEqual(autonomous.meta.routeModeContract.buildMode, 'adaptive_blueprint');
  assert.ok(autonomous.executionMessage.includes('site institucional genérico'));
  ...
  const autonomousBlueprint = blueprintService.buildProjectBlueprintOperationBatch({
    ...
    mediaAssets: {
      hero: {
        ...
        src: 'https://images.pexels.com/photos/institutional.jpeg',
        alt: 'Warm editorial institutional workspace',
        attribution: 'Foto de Faber no Pexels',
        sourceUrl: 'https://www.pexels.com/photo/institutional/',
      },
    },
  });
  assert.strictEqual(autonomousBlueprint.action.blueprint.briefingContract.domain, 'general-institutional-site');
  ...
  assert.deepStrictEqual(autonomousBlueprint.action.blueprint.icons.names, ['documentText', 'sparkles', 'users']);
  assert.strictEqual(autonomousBlueprint.action.blueprint.media.query, 'warm editorial institutional website natural workspace');
  ...
  assert.strictEqual(greenhouseMode.mode, 'initial_blueprint');
  assert.strictEqual(greenhouseMode.reason, 'simple_initial_project_blueprint');
  ...
  const greenhouseRoute = await service.resolveProductRoute({
    projectInfo: createProjectInfo(),
    userMessage: [
      'Criar landing page em Next.js para estufas agrícolas com hero video full width.',
      'Usar verde profundo, off-white, modelos de estufa, FAQ e formulário de orçamento.',
  ...
  assert.strictEqual(greenhouseRoute.decision, 'execute');
  assert.strictEqual(greenhouseRoute.productRoute.mode, 'faber_blueprint');
  assert.strictEqual(greenhouseRoute.meta.reason, 'faber_blueprint_create');
  assert.strictEqual(greenhouseRoute.meta.buildMode, 'initial_blueprint');
  assert.strictEqual(greenhouseRoute.buildModeRoute.mode, 'initial_blueprint');
  assert.strictEqual(greenhouseRoute.meta.routeModeContract.productRouteMode, 'faber_blueprint');
  assert.strictEqual(greenhouseRoute.meta.routeModeContract.buildMode, 'initial_blueprint');

  const temporaryContractRoute = await service.resolveProductRoute({
    projectInfo: createProjectInfo(),
    userMessage: [
    ...
    contextText: 'Memoria antiga sobre Jardim Vivo, paisagismo e loja de plantas.',
  });
  assert.strictEqual(temporaryContractRoute.decision, 'execute');
  assert.strictEqual(temporaryContractRoute.productRoute.mode, 'faber_blueprint');
  assert.strictEqual(temporaryContractRoute.meta.buildMode, 'initial_blueprint');
  assert.strictEqual(temporaryContractRoute.workingBrief.source.memoryContextSuppressed, true);
  assert.strictEqual(temporaryContractRoute.workingBrief.source.consolidated.includes('Jardim Vivo'), false);
  assert.strictEqual(temporaryContractRoute.workingBrief.temporaryBlueprintContract.status, 'active');
  assert.ok(temporaryContractRoute.workingBrief.product.domain.startsWith('temporary-'));

  const temporaryContractBlueprint = blueprintService.buildProjectBlueprintOperationBatch({
    projectInfo: createProjectInfo(),
    userMessage: temporaryContractRoute.executionMessage,
    executionIntent: 'init_project',
    force: true,
    workingBrief: temporaryContractRoute.workingBrief,
    buildModeRoute: temporaryContractRoute.buildModeRoute,
    mediaAssets: {
      hero: {
        kind: 'photo',
        provider: 'pexels',
        query: temporaryContractRoute.workingBrief.mediaIntent[0].query,
        src: 'https://images.pexels.com/photos/music-school.jpeg',
        alt: 'Experimental music class',
        attribution: 'Foto de Faber no Pexels',
        sourceUrl: 'https://www.pexels.com/photo/music-school/',
      },
    },
  });
  const temporaryPaths = temporaryContractBlueprint.action.operations.map((operation) => operation.path);
  const temporaryContent = temporaryContractBlueprint.action.operations.map((operation) => operation.content || '').join('\n');
  assert.strictEqual(temporaryContractBlueprint.action.blueprint.layoutRecipe.id, 'temporary-contract-site');
  assert.strictEqual(temporaryContractBlueprint.action.blueprint.moduleContract.status, 'temporary_contract_resolved');
  assert.strictEqual(
    temporaryContractBlueprint.action.blueprint.moduleContract.visualContracts.responsiveNavigation.hamburgerPlacement,
    'header_right_absolute'
  );
  assert.strictEqual(temporaryContractBlueprint.action.blueprint.coverageContract.status, 'passed');
  assert.ok(temporaryPaths.includes('app/professores/page.tsx'));
  assert.ok(temporaryPaths.includes('app/agenda/page.tsx'));
  assert.ok(temporaryContent.toLowerCase().includes('escola de musica experimental'));
  assert.ok(temporaryContent.includes('/professores'));
  assert.ok(temporaryContent.includes('/agenda'));
  assert.ok(temporaryContent.includes('lg:hidden'));
  assert.ok(temporaryContent.includes('lg:flex'));
  assert.ok(temporaryContent.includes('absolute right-5'));
  assert.strictEqual(temporaryContent.includes('md:hidden'), false);
  assert.strictEqual(temporaryContent.includes('Jardim Vivo'), false);
