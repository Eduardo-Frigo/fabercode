...
async function run() {
  ...
  assert.strictEqual(remoteBudget.generationOptions.num_predict, 12000);

  const remoteHighBudget = darwinService.buildRuntimeBudget(
    { ...createRuntimeSettings('profundo'), generationOptions: { num_predict: 20000 } },
    { pressure: 'normal', freeMb: 12000, swapUsedMb: 0 },
    'openai'
  );
  assert.strictEqual(remoteHighBudget.generationOptions.num_predict, 20000);
