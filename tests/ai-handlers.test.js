...
async function run() {
  ...
  assert.strictEqual(initialSettings.welcomeQuoteLastAuthor, '');
  ...
  const quoteSaved = await handlers['ai:settings:save'](null, {
    welcomeQuoteLastAuthor: 'Sócrates',
  });
  assert.strictEqual(quoteSaved.ok, true);
  assert.strictEqual(quoteSaved.provider, 'openai');
  assert.strictEqual(quoteSaved.welcomeQuoteLastAuthor, 'Sócrates');
