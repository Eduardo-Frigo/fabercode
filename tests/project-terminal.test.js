...
(async () => {
  ...
  try {
    ...
    // Alguns ambientes de teste bloqueiam symlink; o restante da suíte cobre o confinamento.
  }
  ...
  if (fs.existsSync(path.join(projectRoot, 'outside-link'))) {
    const blockedSymlinkCd = service.runCommand({
      rootPath: projectRoot,
      sessionId: created.session.id,
      command: 'cd ../outside-link',
      sendEvent: (event) => events.push(event),
    });
    assert.strictEqual(blockedSymlinkCd.ok, false);
    assert.match(blockedSymlinkCd.message, /Diretório físico fora da raiz/);
  }
