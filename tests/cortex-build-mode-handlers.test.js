...
function run() {
  assert.deepStrictEqual(
    ...
    ['tool_action', 'diagnostic_repair', 'design_to_code', 'visual_review']
  );
  ...
  const visualReview = resolveCortexBuildModeRoutePayload({
    buildModeRoute: { mode: 'visual_review', confidence: 0.82 },
    routedExecutionMessage: 'validar screenshots',
    projectState: 'existing_project',
  });
  assert.strictEqual(visualReview.reason, 'build_mode_visual_review');
  assert.strictEqual(visualReview.decision, 'chat');
  assert.strictEqual(visualReview.capability, 'diagnose_project');
  assert.strictEqual(visualReview.executionIntent, 'visual_review');
  assert.strictEqual(visualReview.extraMeta.noFileChanges, true);
