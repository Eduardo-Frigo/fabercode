...
function writeLocalPackage(rootPath, name) {
  const target = path.join(rootPath, 'node_modules', name, 'package.json');
  fs.mkdirSync(path.dirname(target), { recursive: true });
  fs.writeFileSync(target, JSON.stringify({ name, version: '0.0.0-test' }), 'utf8');
}
...
async function testNextCreationPreviewVerificationAndEditing(tempRoot) {
  ...
  ['next', 'react', 'react-dom'].forEach((name) => writeLocalPackage(rootPath, name));
  ...
  assert.ok(footerPatch.action.operations[0].content.includes('<BlueprintFooterUtility brand='));
  assert.ok(footerPatch.action.operations[0].content.includes('tone="dark"'));

  const brokenPage = `${fs.readFileSync(path.join(rootPath, 'app', 'page.tsx'), 'utf8')}

const baseStyles = "border border-slate-200 rounded-lg px-3 py-2 bg-white";
const style = document.createElement("style");
style.innerHTML = \`
  .input { \${baseStyles}; }
  .btn { \${baseStyles}; background: #1f2937; color: white; font-weight: 600; }
  .table { width: 100%; border-collapse: collapse; }
\`;
if (typeof document !== "undefined" && !document.head.querySelector("style[data-forge-mrp]") ) {
  style.setAttribute("data-forge-mrp", "true");
  document.head.appendChild(style);
}
`;
  fs.writeFileSync(path.join(rootPath, 'app', 'page.tsx'), brokenPage, 'utf8');
  const browserGlobalPatch = deterministicService.buildContentEditOperationBatch({
    projectInfo,
    executionIntent: 'edit_project',
    userMessage:
      'Corrija o preview Next.js que falha com ReferenceError: document is not defined removendo document em escopo de módulo.',
  });
  assert.strictEqual(browserGlobalPatch.ok, true);
  assert.strictEqual(browserGlobalPatch.action.generatedBy, 'deterministic_next_browser_global_module_patch');
  assert.deepStrictEqual(browserGlobalPatch.action.operations.map((operation) => operation.path), [
    'app/page.tsx',
    'app/globals.css',
  ]);
  assert.strictEqual(browserGlobalPatch.action.operations[0].content.includes('document.createElement'), false);
  assert.ok(browserGlobalPatch.action.operations[1].content.includes('faber-next-browser-global-module-repair'));
  applyOperations(rootPath, browserGlobalPatch.action.operations);
  assert.strictEqual(
    /document\.createElement|document\.head/.test(fs.readFileSync(path.join(rootPath, 'app', 'page.tsx'), 'utf8')),
    false
  );

  fs.mkdirSync(path.join(rootPath, 'lib'), { recursive: true });
  fs.writeFileSync(
    path.join(rootPath, 'lib', 'mrp.ts'),
    [
      'export function audit(payload: Record<string, unknown>) {',
  ...
  projectInfo.files.push('lib/mrp.ts');
  projectInfo.totalFiles = projectInfo.files.length;
  const structuredClonePatch = deterministicService.buildContentEditOperationBatch({
    projectInfo,
    executionIntent: 'edit_project',
    userMessage: 'Corrija os testes Jest: ReferenceError structuredClone is not defined em lib/mrp.ts.',
  });
  assert.strictEqual(structuredClonePatch.ok, true);
  assert.strictEqual(structuredClonePatch.action.generatedBy, 'deterministic_structured_clone_compat_patch');
  assert.strictEqual(structuredClonePatch.action.operations[0].path, 'lib/mrp.ts');
  assert.ok(structuredClonePatch.action.operations[0].content.includes('function cloneSerializable<T>'));
  assert.strictEqual(structuredClonePatch.action.operations[0].content.includes('structuredClone(payload)'), false);
  assert.strictEqual(
    deterministicService.classifySafeContentEditRequest(
      'Corrija os testes Jest: ReferenceError structuredClone is not defined em lib/mrp.ts.'
    ).kind,
    'structured_clone_compat_micro_patch'
  );
}
