...
function sanitizeWelcomeQuoteLastAuthor(rawValue) {
...
function createAiRuntimeSettingsService(dependencies = {}) {
  ...
  function getDefaultProvider() {
    return normalizeAiProviderName(aiProviderEnv || 'mock');
  }
  ...
  function getSafeFallbackProvider(disabledBuiltInProviders = []) {
    ...
    return fallback.startsWith('custom:') || disabled.has(fallback) ? 'mock' : fallback;
  }
  ...
  function readSettings() {
    ...
    try {
      ...
      return {
        ...
        customApis,
        disabledBuiltInProviders,
        welcomeQuoteLastAuthor: sanitizeWelcomeQuoteLastAuthor(parsed && parsed.welcomeQuoteLastAuthor),
      };
    } catch {
      return {
  ...
  function writeSettings(settings = {}) {
    ...
    const hasWelcomeQuoteLastAuthor = Object.prototype.hasOwnProperty.call(settings, 'welcomeQuoteLastAuthor');
    ...
    const next = {
      ...
      customApis,
      disabledBuiltInProviders,
      welcomeQuoteLastAuthor: hasWelcomeQuoteLastAuthor
        ? sanitizeWelcomeQuoteLastAuthor(settings.welcomeQuoteLastAuthor)
        : sanitizeWelcomeQuoteLastAuthor(current.welcomeQuoteLastAuthor),
    };
