...
const REMOTE_PROVIDER_MIN_NUM_PREDICT = 12000;
...
function createCortexRuntimeBudgetService(dependencies = {}) {
  ...
  function buildGenerationOptionsForProvider(runtimeSettings = {}, provider = '') {
    const baseOptions =
      runtimeSettings && runtimeSettings.generationOptions && typeof runtimeSettings.generationOptions === 'object'
        ? { ...runtimeSettings.generationOptions }
        : {};
    if (!isRemoteProvider(provider)) return baseOptions;

    const currentNumPredict = Number(baseOptions.num_predict);
    baseOptions.num_predict = Math.max(
      Number.isFinite(currentNumPredict) ? currentNumPredict : 0,
      REMOTE_PROVIDER_MIN_NUM_PREDICT
    );
    return baseOptions;
  }
  ...
  function buildRuntimeBudget(runtimeSettings, memoryState = buildBasicMemoryState(), provider = 'rwkv') {
    ...
    const generationOptions = buildGenerationOptionsForProvider(runtimeSettings, selectedProvider);
    return {
  ...
  return {
    ...
    buildRuntimeBudget,
    buildGenerationOptionsForProvider,
