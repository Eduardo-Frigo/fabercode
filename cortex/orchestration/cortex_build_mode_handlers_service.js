const CORTEX_POLICY_OWNED_BUILD_MODES = new Set(['tool_action', 'diagnostic_repair', 'design_to_code', 'visual_review']);
...
function buildVisualReviewRoutePayload({
  buildModeRoute = null,
  provider = 'deterministic',
  projectState = '',
} = {}) {
  return {
    decision: 'chat',
    response:
      'Entendi. Vou tratar isso como validação visual/diagnóstico, sem converter os screenshots em uma referência para editar arquivos. Os anexos entram como evidência; mudanças só devem acontecer em um pedido separado de correção.',
    executionMessage: '',
    confidence: Math.max(Number(buildModeRoute && buildModeRoute.confidence ? buildModeRoute.confidence : 0), 0.9),
    reason: 'build_mode_visual_review',
    provider,
    capability: 'diagnose_project',
    mode: 'visual_review',
...
const CORTEX_BUILD_MODE_HANDLERS = [
  ...
  {
    mode: 'visual_review',
    canHandle: ({ buildModeRoute } = {}) => getBuildMode(buildModeRoute) === 'visual_review',
    buildRoutePayload: buildVisualReviewRoutePayload,
  },
];
