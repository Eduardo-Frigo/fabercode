function createMainRuntimeConfig({
  ...
  const PERSONA_MODEL_ENGINE = env.PERSONA_MODEL_ENGINE || 'mock-persona';
  const PERSONA_MODEL_BRAIN = env.PERSONA_MODEL_BRAIN || 'mock-persona';
  ...
  const RWKV_ENABLED = String(env.RWKV_ENABLED || 'false').toLowerCase() === 'true';
  ...
  const CORTEX_MEMORY_EMBEDDING_PROVIDER = String(env.CORTEX_MEMORY_EMBEDDING_PROVIDER || 'local').trim().toLowerCase();
  const CORTEX_MEMORY_EMBEDDING_ENDPOINT = String(
    env.CORTEX_MEMORY_EMBEDDING_ENDPOINT || env.OPENAI_EMBEDDING_ENDPOINT || ''
  ).trim();
  const CORTEX_MEMORY_EMBEDDING_API_KEY = String(
    env.CORTEX_MEMORY_EMBEDDING_API_KEY || env.OPENAI_EMBEDDING_API_KEY || env.OPENAI_API_KEY || ''
  ).trim();
  const CORTEX_MEMORY_EMBEDDING_MODEL = String(
    env.CORTEX_MEMORY_EMBEDDING_MODEL || env.OPENAI_EMBEDDING_MODEL || 'text-embedding-3-small'
  ).trim();
  const CORTEX_MEMORY_EMBEDDING_DIMENSIONS = Number.parseInt(
    env.CORTEX_MEMORY_EMBEDDING_DIMENSIONS || '1536',
    10
  );
  const CORTEX_MEMORY_EMBEDDING_TIMEOUT_MS = Number.parseInt(
    env.CORTEX_MEMORY_EMBEDDING_TIMEOUT_MS || '12000',
    10
  );
  ...
  const R2R_CORTEX_REINDEX_ENDPOINT = String(env.R2R_CORTEX_REINDEX_ENDPOINT || '').trim();
  const R2R_CORTEX_DELETE_ENDPOINT = String(env.R2R_CORTEX_DELETE_ENDPOINT || env.R2R_CORTEX_FORGET_ENDPOINT || '').trim();
