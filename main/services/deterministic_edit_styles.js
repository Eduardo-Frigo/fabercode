...
function extractBackgroundColor(userMessage = '') {
  ...
  return backgroundMatch && backgroundMatch[1] ? expandShortHex(backgroundMatch[1]) : '';
}
...
function updateBackgroundColorCss(content, color) {
  ...
  const variableResult = replaceCssVariableIfPresent(next, ['--color-bg', '--bg', '--background'], normalizedColor);
